...
-- =========================================
-- trigger: Chỉ khám bệnh mới được thêm vào dịch vụ khám bệnh
-- =========================================
create or alter trigger trg_check_loaidv_kham
on chitietkhambenh
after insert, update
as
begin
    if exists (
        ...
        where dv.loai <> N'Khám bệnh'
    )
    begin
        ;throw 50006, N'Chỉ dịch vụ "Khám bệnh" được dùng trong bảng này.', 1
    end
end
go

-- Tương tự cho tiêm phòng
create or alter trigger trg_check_loaidv_tiem
on chitiettiemphong
after insert, update
as
begin
    if exists (
        ...
        from inserted i
        join dichvu dv on i.madv = dv.madv
        where dv.loai <> N'Tiêm phòng'
    )
    begin
        ;throw 50007, N'Chỉ dịch vụ "Tiêm phòng" được dùng trong bảng này.', 1
    end
end
go